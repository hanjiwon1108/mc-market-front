...
const fetcher = async ([session, u]: [Session, string]) => {
  ...
  return (await it.json()) as Promise<AdcardType[]>;
};

export default function Page() {
  const session = useSession();
  const infinite = useSWRInfinite(
    (index, previousPageData: AdcardType[]) => {
      if (previousPageData && !previousPageData.length) return;
      if (index == 0) {
        return [session, endpoint('/v1/adcard/list/')];
      }
      const lastHead = previousPageData.reduce((p, c) => (p.id > c.id ? p : c));

      return [
        session,
        `${endpoint(`/v1/adcard/`)}?offset=${index == 0 ? 0 : lastHead.id}`,
      ];
    },
    {
      fetcher: fetcher,
  ...
  const deleteAdcard = async (id: number) => {
    if (!confirm('정말로 삭제하시겠습니까?')) {
      return;
    }
    const response = await authFetch(
      session,
      endpoint(`/v1/adcard/delete/${id}`),
      {
        method: 'POST',
      },
    );
    if (response.ok) {
      toast.success('adcard를 삭제했습니다');
    } else {
      toast.error('adcard 삭제에 실패했습니다');
    }
  };

  const uploadAdcard = async () => {
    ...
    const response = await authFetch(session, endpoint(`/v1/adcard/upload`), {
      method: 'POST',
      body: formData,
    });

    if (response.ok) {
      toast.success('adcard를 생성했습니다');
    } else {
      toast.error('adcard 생성에 실패했습니다');
    }
  };

  const updateAdcard = async (id: number) => {
    ...
    if (response.ok) {
      toast.success('adcard를 수정했습니다');
    } else {
      toast.error('adcard 수정에 실패했습니다');
    }
  };
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow>
            ...
            <TableCell>
              <Button size="sm" variant="secondary" onClick={uploadAdcard}>
                생성
              </Button>
            </TableCell>
          </TableRow>
